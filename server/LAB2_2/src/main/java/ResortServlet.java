...
public class ResortServlet extends HttpServlet {
    ...
    private static String IP_REDIS = "172.31.26.25";
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try (Jedis jedis = pool.getResource()) {
            String resortKey = "" + resortId;
            if (!jedis.sismember("resorts", resortKey)) {
                res.setStatus(HttpServletResponse.SC_NOT_FOUND);
                res.getWriter().write(Utils.getReturnMessage(gson, "resort not exists"));
                return;
            }
            res.setStatus(HttpServletResponse.SC_OK);
            String key = "re" + resortId + "s" + seasonId + "d" + daysId + "users";
            Long num = jedis.scard(key);
            ResortResponse response = new ResortResponse(resortKey, num.intValue());
    ...
    private int isUrlValid(String[] urlPath, String source) {
        ...
        if (source.equals("GET")) {
            ...
            if (urlPath.length == 7) {
                ...
                if (!urlPath[4].equals("day")) return -1;
